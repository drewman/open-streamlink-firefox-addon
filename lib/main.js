...
// The universal button for Open With Streamlink
...
// A gernic panel where all content is showed for Open With Streamlink
...
// It then grabs the current url and has getStreamlinkValidation()
// validate the url + get the quality posibilites.
// Inside of getStreamlinkValidation() ContentScript is pass to populate the
// quality buttons.
function openPanel(state) {
    ...
    if (state.checked) {
        ...
        if (prefs.qualityselector) {
            // Get current tab url
            getStreamlinkValidation(tabs.activeTab.url);
        } else {
            runStreamlink(buildArgs(tabs.activeTab.url));
        }
        panel.show({
            position: button
        });
...
// Context Menu
// Right-click -> "Open with Streamlink"
    ...
    contextMenu.Item({
       label: locale("label"),
        ...
        onMessage: function(streamURL) {
            // Display quality menu?
            if (prefs.qualityselector) {
                getStreamlinkValidation(streamURL);
            } else {
                runStreamlink(buildArgs(streamURL));
            }
            panel.show({
                position: button
            });
...
function getStreamlinkPath() {
    ...
    // Best guess
        ...
        if (system.architecture == 'x86') {
            path = "C:\\Program Files\\Streamlink\\streamlink.exe";
        } else {
            /* Assume 'x86_64' by default */
            path = "C:\\Program Files (x86)\\Streamlink\\streamlink.exe";
        }
    } else if (system.platform == "mac") {
        // assume homebrew install
        path = "/usr/local/bin/streamlink";
    }
    // Test file
    file.initWithPath(path);
    if (file.exists()) {
        ...
        panel.port.emit("status", locale("status_error_streamlink"));
        return null;
    }
}

// Runs streamlink and validates the url
...
function getStreamlinkValidation(url) {
    var path,
        process,
        streamlink,
        jsonData = '',
    ...
    // Spawn a seperate child to validate in Streamlink
    streamlink = process.spawn(path, ["--json", url]);
    // As the stream comes in, grab the data
    streamlink.stdout.on('data', function(data) {
        // Notify
        panel.width = 250;
        panel.height = 97;
